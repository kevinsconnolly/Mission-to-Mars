...
def scrape_all():
    # Start headless driver for deployment
...
def cerberus_images(browser):
    ...
    browser.visit(url)
    # Find and click  hemisphere button/link
    image_elem = browser.find_by_css('a.product-item h3')
    image_elem.click()
    # separate the result of html with soup
    html = browser.html
    img_soup = BeautifulSoup(html, 'html.parser')
    # Find title
    title_elem= img_soup.find("h2", class_="title").get_text()
    title_elem
    # Find sample button & click
    ...
    try:
        ...
        return None
    # Use the base URL to create an abs(URL)
...
def schiaparelli_images(browser):
    ...
    url = 'https://astrogeology.usgs.gov/search/results?q=hemisphere+enhanced&k1=target&v1=Mars'
    browser.visit(url)
    # Find & click the hemisphere link button
    image_elem = browser.find_by_css('a.product-item h3')
    image_elem.click()
    # Parse the resulting html with soup
    html = browser.html
    ...
    try:
        ...
        return None
    # Use base URL to create an abs(URL)
    img_url = f'https://astrogeology.usgs.gov{img_url_rel}'
...
def syrtis_images(browser):
    ...
    image_elem = browser.find_by_css('a.product-item h3')
    image_elem.click()
    # Separate resulting html with soup
    html = browser.html
    img_soup = BeautifulSoup(html, 'html.parser')
    # Find the title
    title_elem= img_soup.find("h2", class_="title").get_text()
